...
func (t Training) distance() float64 {
	// вставьте ваш код ниже Action * LenStep / MInKm
	if t.LenStep > 0 {
...
func (w Walking) Calories() float64 {
	...
	return (CaloriesWeightMultiplier*w.Weight + (meanSpeedMS/(w.Height/CmInM))*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours
}
...
type Swimming struct {
	// добавьте необходимые поля в структуру
	Training
	LengthPool, CountPool int // длина бассейна и количество пересечений бассейна
}
